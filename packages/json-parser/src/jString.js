...
// String
const jUnescapedChar = satisfy(ch => ch !== '\\' && ch !== '"').setLabel(
  'char'
);
...
const jUnicodeChar = backslash
  .andThenRight(uChar)
  .andThenRight(hexdigit)
  .andThen(hexdigit)
  .andThen(hexdigit)
  .andThen(hexdigit)
  .map(convertToChar);
...
export const quotedString = quote
  .andThenRight(manyChars(jchar))
  .andThenLeft(quote);
